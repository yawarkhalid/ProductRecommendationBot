...
class Product_Information_Retrieval_Model():
    ...
    def __init__(self):
        ...
        self.df = df = pd.read_excel("data/fashion_final.xlsx")
        self.hal = sia()
    ...
    def query_similar_products(self, message, n):

        love_message, hate_message = self.get_message_sentiment(message)

        similar_products = self.get_ensemble_similarity_scores(love_message)
        dissimilar_products = self.get_dissimilarity_scores(hate_message)
        dissimilar_products = dissimilar_products.query('dissimilarity > .3')
        similar_products = similar_products.drop(dissimilar_products.index)
    ...
    def view_recommendations(self, recs):
        ...
        for i in range(len(recs)):
            ...
            single_product = self.df.query('title==@single_title')
            name = single_product.title.values[0]
            notes = single_product.notes.values[0]
